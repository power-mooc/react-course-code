...
export type WatermarkOptions = Omit<WatermarkProps, 'style' | 'className' | 'children'>;

export const isNumber = (obj: unknown): boolean => {
...
const toNumber = (value?: string | number, defaultValue?: number) => {
  ...
  const numberVal = parseFloat(String(value));
